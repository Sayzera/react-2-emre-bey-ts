...
const NotificationComponent: React.FC<NotificationProps> = ({ notification, onRemove }) => {
  ...
  return (
    <div className={`${bgColors[notification.type]} border rounded-lg p-4 shadow-lg mb-3 flex items-start gap-3 animate-slide-in`}>
...
const NotificationSystem: React.FC = () => {
  ...
  const addNotification = (type: NotificationType, title: string, message: string): void => {
    dispatch({
  ...
  // Her bildirim 3 saniye sonra otomatik silinir
  useEffect(() => {
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 mb-6">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h1 className="text-3xl font-bold text-gray-800">Bildirim Yönetim Sistemi</h1>
          </div>
          <p className="text-gray-600">useReducer + TypeScript ile yapılmış bildirim sistemi - Her bildirim 3 saniye sonra otomatik silinir</p>
        </div>

        {/* Butonlar */}
        <div className="bg-white rounded-xl shadow-lg p-6 mb-6">
          <h2 className="text-xl font-semibold text-gray-800 mb-4">Bildirim Ekle</h2>
          <div className="flex flex-wrap gap-3">
            <button
              onClick={() => addNotification('success', 'Başarılı!', 'İşlem başarıyla tamamlandı.')}
              className="bg-green-500 hover:bg-green-600 text-white px-6 py-3 rounded-lg font-semibold transition-colors shadow-md"
            >
              ✓ Başarı Bildirimi
            </button>
            <button
              onClick={() => addNotification('error', 'Hata!', 'Bir şeyler yanlış gitti.')}
              className="bg-red-500 hover:bg-red-600 text-white px-6 py-3 rounded-lg font-semibold transition-colors shadow-md"
            >
              ✗ Hata Bildirimi
            </button>
            <button
              onClick={() => addNotification('info', 'Bilgi', 'Yeni bir güncelleme mevcut.')}
        ...
        <div className="bg-white rounded-xl shadow-lg p-4 mb-6">
          <div className="flex items-center justify-between">
            <span className="text-gray-700 font-medium">Aktif Bildirimler:</span>
        ...
        {notifications.length === 0 && (
          <div className="bg-white rounded-xl shadow-lg p-8 text-center">
            ...
            <p className="text-gray-400 text-sm mt-2">Yukarıdaki butonları kullanarak bildirim ekleyin</p>
          </div>
        )}
      </div>
